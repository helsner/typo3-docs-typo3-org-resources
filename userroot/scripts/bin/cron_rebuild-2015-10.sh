...
# Exit Status:
#
# 1: Configuration file buildsettings.sh was not found
...
# 6: Configuration file $T3DOCDIR/Settings.cfg was not found
# ------------------------------------------------------

# Retrieve current script directory (as absolute path)
# BIN_DIRECTORY=$(dirname $(readlink "$0"))

# Better define as constant. Don't require symlink.
BIN_DIRECTORY=/home/mbless/scripts/bin
...
if [ ! -r "buildsettings.sh" ]; then
    echo "Cannot find configuration file buildsettings.sh in $MAKE_DIRECTORY" 2>&1
    exit 1
fi
...
cp buildsettings.sh dirs-of-last-build.txt
